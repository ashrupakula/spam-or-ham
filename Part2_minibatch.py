...
def pre_process():
    ...
    for i in range(len(df)):
        ...
        for de in delimiters: #Breaking sentences into tokens
            df.iloc[i,1] = df.iloc[i,1].replace(de, ' ')  
        ...
        df.iloc[i,4] = []
        for j in df.iloc[i,3]: #Using Porter Stemming
    ...
    uniquewordsfreq = []
    for i in range(len(df)):
        for word in df.iloc[i,4]: #Finding frequency of each word
    ...
    uniquewords = []
    for i in range(500): #Choosing top 500 frequent words
    ...
    onehotvectorlist = []
    y_values = []
    for i in range(len(df)): #Converting entire dataset into 0's and 1's
...
def forward_pass(x0, d, w): #Forwardpass
    ...
    l = len(d)-1
    for j in range(d[l]):
        x[l][j] = (np.sum([w[l-1][i][j]*x[l-1][i] for i in range(d[l-1])]))
    y = softmax(x[l], d[l])
    for j in range(d[l]):
        x[l][j] = y[j]
    return x
...
def back_propogation(d, x, w, y): #Backpropogation
    ...
    lend = len(d)
    for i in range(d[-1]):
        delta[-1][i] = -ydash[i]*(1-x[-1][i])+(1-ydash[i])*(x[-1][i])
    for l in range(1, len(d)-1):
        for i in range(1, d[lend-l-1]):
                delta[lend-l-1][i] = x[lend-l-1][i]*(1-x[lend-l-1][i])*np.dot(w[lend-l-1][i],delta[lend-l])
    return delta

def compute_error_point(x, y):
    if(y==1):
        return -(y*np.log(x[-1][1])+np.finfo(float).eps)
    if(y==0):
        return -((1-y)*np.log(x[-1][0]))
    else:
...
def train(w, X, y, d, epoch):
    ite = 0
    while(ite<len(X)):
        grad = np.array([[[0]*d[l+1] for i in range(d[l])] for l in range(len(d)-1)])
        X_batch = X[ite:ite+BATCH_SIZE, :] #Data_Loader
...
d = [501, int(sys.argv[1])+1, int(sys.argv[2])+1, 2]
...
plt.legend()
plt.savefig("Part2_minibatch_graph.png", bbox_inches = 'tight')

print("Training Error: "+str(train_err))
with open('training_error_minibatch_p2.txt', 'w') as f:
    for item in train_err:
        f.write(str(item)+'\n')
print("Test Error: "+str(test_err))
with open('test_error_minibatch_p2.txt', 'w') as f:
